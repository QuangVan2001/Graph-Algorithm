...
public class Graph_Matrix {
    ...
    private RandomAccessFile creaFile(String fname) throws FileNotFoundException{
    ...
    void visit_F(RandomAccessFile f, int i) throws IOException{
        f.writeBytes(vSet[i] + " ");
    }
    
    void BF_traverseComponent_F(String filename, int k) throws FileNotFoundException, IOException{
        RandomAccessFile rf = creaFile(filename);
        boolean [] visited = createFlags();
        MyQueue q = new MyQueue();
        q.enqueue(k);
        visited[k] = true;
    ...
    void BF_traverseAll_F(String filename) throws FileNotFoundException, IOException{
        RandomAccessFile rf = creaFile(filename);
        boolean [] visited = createFlags();
    ...
    void DFS_F(RandomAccessFile rf, int  v, boolean [] visited) throws IOException{
        visit_F(rf, v);
        visited[v] = true;
        for (int j = 0; j < nVertices; j++) {
            if(!visited[j] && adjMatrix[v][j] >0){
    ...
    void DF_traverseComponent_F(String fifename, int k) throws FileNotFoundException, IOException{
        RandomAccessFile rf = creaFile(fifename);
        boolean [] visited = createFlags();
    ...
    void DF_traverseAll_F(String filename) throws FileNotFoundException, IOException{
        RandomAccessFile rf = creaFile(filename);
        boolean [] visited = createFlags();
